...
class User < ApplicationRecord
  ...
  def pending_friends
    initiated_friendships.map{|friendship| friendship.requestee if !friendship.status}.compact
  end

  def friend_requests
    invited_friendships.map{|friendship| friendship.requestor if !friendship.status}.compact
  end
  ...
  def add_friend(user)
    if !self.friend?(user)
      add_friendship = Friendship.new(requestee_id: user.id, requestor_id: self.id)
  ...
  def remove_friend(user)
    if self.friend?(user) || user.friend_requests.include?(self)
      remove_friendship = Friendship.find_by(requestee_id: user.id, requestor_id: self.id) || Friendship.find_by(requestee_id: self.id, requestor_id: user.id)
